...
namespace Tc.Crm.Service.Client.PerformanceConsole
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                else if (option == "2")
                {
                    ...
                    var t1 = File.ReadAllText("booking.json");
                    Booking bookingInfo = new Booking();
                    using (MemoryStream memoryStream = new MemoryStream(Encoding.UTF8.GetBytes(t1)))
                    {
