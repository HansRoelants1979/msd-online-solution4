...
namespace Tc.Crm.CustomWorkflowSteps.ProcessBooking.Services
{
    public static class BookingHelper
    {
        ...
        public static int GetDurationFrom(Transport[] transports, ITracingService trace)
        {
            ...
            if (string.IsNullOrEmpty(transport.EndDate)) return -1;
            if (string.IsNullOrEmpty(transport.StartDate)) return -1;
            DateTime startDate = DateTime.MinValue;
            DateTime endDate = DateTime.MinValue;
            if (!DateTime.TryParse(transport.StartDate, out startDate))
                return -1;
            if (!DateTime.TryParse(transport.EndDate, out startDate))
                return -1;
