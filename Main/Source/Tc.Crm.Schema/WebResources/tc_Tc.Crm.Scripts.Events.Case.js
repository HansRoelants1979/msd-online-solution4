...
scriptLoader.load("Tc.Crm.Scripts.Events.Case", ["Tc.Crm.Scripts.Utils.Validation", "Tc.Crm.Scripts.Library.Case"], function () {
    ...
    Tc.Crm.Scripts.Events.Case = (function () {
        ...
        var getNewValue = function (tcProductValue) {
        ...
        var setTcProductRelatedFieldValue = function () {
            var surpriseFieldValue = getControlValue(Attributes.TcProductRelated);
            if (surpriseFieldValue == null)
                return;
            Xrm.Page.getAttribute(Attributes.TcProductRelated).setValue(getNewValue(surpriseFieldValue));
        }

        var tcProductRibbonButtonEnabled = false;
        ...
        var ribbonButtonRefreshed = false;

        var refreshRibbon = function (userRoleId) {
            try {
                if (!ribbonButtonRefreshed) {
                    ...
                    Tc.Crm.Scripts.Common.GetById(EntitySetNames.SecurityRole, userRoleId, query).then(
                        function(securityRoleNameResponse) {
                            ...
                            if (role == null || role == "" || role == "undefined") return null;

                            if (role.name === SecurityRoles.SystemAdministrator || role.name === SecurityRoles.HealthSafety || role.name === SecurityRoles.SystemMaintenance) {
                                tcProductRibbonButtonEnabled = isIncidentType();
                                Xrm.Page.ui.refreshRibbon();
                                ribbonButtonRefreshed = true;
                            }
        ...
        return {
            ...
            OnSave: function (context) {
                ...
                if (isValid) {
                    if (Xrm.Page.context.client.getClientState() !== ClientState.Offline) {
